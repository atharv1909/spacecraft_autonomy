...
def _corridor_violation(traj, hard_radius, cone_half_angle_deg, corridor_radius):
    x = traj[..., 0]
    r_transverse = np.sqrt(traj[..., 1] ** 2 + traj[..., 2] ** 2)
    dist = np.linalg.norm(traj, axis=-1)

    off_axis_deg = np.degrees(np.arctan2(r_transverse, np.maximum(np.abs(x), 1e-3)))
    in_cone = off_axis_deg <= cone_half_angle_deg

    hard_violation = dist < hard_radius
    corridor_violation = (~in_cone) & (dist < corridor_radius)

    return np.any(hard_violation | corridor_violation, axis=1)
...
class DigitalTwin:
    ...
    def propagate(self, initial_state: np.ndarray, action: str,
        ...
        # Metrics
        min_dist = np.min(np.linalg.norm(traj, axis=2), axis=1)  # keep for reporting
        collision = _corridor_violation(
            traj, self.sim.cfg.hard_body_radius,
            self.sim.cfg.approach_cone_half_angle_deg, self.sim.cfg.keepout_radius
        )
