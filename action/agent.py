...
class ActionAgent:
    ...
    def _format_payload(self, results: list) -> Dict:
            ...
            recommended = best['action'] if best else 'hold_position'
            score = round(best['score'], 4) if best else 0.0
            
            p_mean_primary = actions_out[0]['metrics']['collision_probability']['mean'] if actions_out else 0.0
            p_upper_primary = actions_out[0]['metrics']['collision_probability']['guaranteed_upper_bound_99pct'] if actions_out else 0.0
            ...
            return {
                ...
                'payload': {
                    ...
                    'primary_score': score,
                    'collision_prob': p_mean_primary,
                    'collision_prob_upper_bound_99': p_upper_primary,
                    'actions': actions_out,
                    'alternatives': [
                        {
                            'action': a['name'],
                            'score': a['score'],
                            'collision_prob': a['metrics']['collision_probability']['mean'],
                            'collision_prob_upper_bound_99': a['metrics']['collision_probability']['guaranteed_upper_bound_99pct']
                        }
                        for a in actions_out[1:]
                    ] if len(actions_out) > 1 else [],
                    'simulation_horizon_s': 60,
                    'mc_runs': results[0]['metrics']['tactical']['trajectories'].shape[0] if results else 0,
                    'explanation': f"Recommended {recommended} based on Monte-Carlo counterfactual simulation (collision upper bound 99% CI: {p_upper_primary:.1%})."
                }
            }
