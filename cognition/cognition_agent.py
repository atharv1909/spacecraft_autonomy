...
class ExplainabilityInterface:
    ...
    def generate_narrative(self, decomposition: Dict[str, float],
        ...
        prefix = f"{root_cause_narrative} " if root_cause_narrative else ""
        ...
        if top_cases:
            ...
            sim = best[1]
            narrative = (
                f"{prefix}System detected {anomaly_data.get('failure_type', 'anomaly')} "
...
class HyperdimensionalCognitionLayer:
    ...
    def process(self, 
        ...
        # 6. Root-cause causal analysis
        sev_rank = {"nominal": 0, "degraded": 1, "critical": 2, "failed": 3}
        states = telem_encoded["states"]
        
        # Aggregate sub-states into top-level subsystems
        thermal_sub = [states.get("radiator", "nominal")]
        power_sub = [states.get("battery", "nominal"), states.get("solar", "nominal")]
        life_sub = [states.get("o2", "nominal"), states.get("co2", "nominal"), states.get("pressure", "nominal")]
        
        thermal_sev = max(thermal_sub, key=lambda s: sev_rank.get(s, 0))
        power_sev = max(power_sub, key=lambda s: sev_rank.get(s, 0))
        life_sev = max(life_sub, key=lambda s: sev_rank.get(s, 0))
        
        # Override with explicit anomaly report if present
        if anomaly_report.severity in sev_rank and anomaly_report.severity != "nominal":
            if "thermal" in anomaly_report.failure_type.lower():
                thermal_sev = anomaly_report.severity
            elif "power" in anomaly_report.failure_type.lower():
                power_sev = anomaly_report.severity
            elif "pressure" in anomaly_report.failure_type.lower() or "o2" in anomaly_report.failure_type.lower():
                life_sev = anomaly_report.severity

        subsystem_states = {
            "thermal": SubsystemState("thermal", thermal_sev),
            "power": SubsystemState("power", power_sev),
            "life_support": SubsystemState("life_support", life_sev)
        }
        root_cause_res = find_root_cause(subsystem_states)
        ...
        anomaly_meta = {
            ...
            "root_cause": root_cause_res.get("root_cause"),
            "root_cause_narrative": root_cause_res.get("narrative", "")
        }
        ...
        # 8. Package output for Phase 3/5
        result = {
            "agent_id": "cognition",
            ...
            "payload": {
                ...
                "root_cause": root_cause_res.get("root_cause") or "",
                "root_cause_narrative": root_cause_res.get("narrative") or "",
                "subsystem_states": {k: v.severity for k, v in subsystem_states.items()},
                "explanation": {
                    "narrative": narrative,
